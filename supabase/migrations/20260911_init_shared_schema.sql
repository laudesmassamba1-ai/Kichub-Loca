-- Full shared schema for Kichub Loca (cloud bootstrap)
...
drop policy if exists commerces_read_all on public.commerces;
create policy "commerces_read_all" on public.commerces
for select using (auth.role() = 'authenticated');

drop policy if exists commerces_insert_own on public.commerces;
...
drop policy if exists commerces_delete_own on public.commerces;
create policy "commerces_delete_own" on public.commerces
for delete using (cree_par = auth.uid());

drop policy if exists visites_select_authenticated on public.visites;
...
drop policy if exists visites_delete_own on public.visites;
